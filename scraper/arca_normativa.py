...
def parse_norma(html: str, norma_id: str) -> dict:
    ...
    sancion_match = re.search(r"Sanci[oó]n:\s*\n*\s*([\d\-]{8,10})", html)
    fecha_sancion = sancion_match.group(1) if sancion_match else None

    publicacion_match = re.search(r"Publicada en el Bolet[ií]n Oficial:\s*\n*\s*\[?([\d\-]{8,10})", html)
    fecha_publicacion = publicacion_match.group(1) if publicacion_match else None
