...
function detectarPlazosVencidos(texto, fechaIso, hoyIso) {
  ...
  while ((m = re.exec(texto)) !== null) {
    ...
    if (iso < hoyIso) encontrados.add(`${dia} de ${m[2].toLowerCase()} de ${anio}`);
  }
  return [...encontrados];
}
...
function armarPromptSistema(contexto) {
  ...
  const hoyIso = new Date().toLocaleDateString("en-CA", { timeZone: "America/Argentina/Buenos_Aires" }); // YYYY-MM-DD
  ...
  const bloques = contexto
    .map((c) => {
      ...
      const avisoPlazo = plazos.length
        ? `\n[AVISO: este texto menciona plazos que YA VENCIERON respecto de hoy (${hoy}): hasta el ${plazos.join("; hasta el ")}. No lo presentes como abierto: decí que ese plazo ya cerró (con la fecha) y que hay que consultar al SENASA por la próxima convocatoria.]`
        : "";
      return `[Fuente: ${cita}${fecha}]${avisoDerogacion}${avisoPlazo}\n${limpiarTexto(c.texto)}`;
    })
    .join("\n\n");
...
--- FIN DEL CONTEXTO ---
Recordatorio final: hoy es ${hoy}. Hablale de vos. No escribas números de fragmento. No presentes como abierto ningún plazo que ya venció. Respondé solo con lo que está en las fuentes de arriba.`;
}
