  ...
  const bloques = contexto
    .map((c, i) => {
      ...
      const avisoPlazo = plazos.length
        ...
        : "";
      return `[S${i + 1}: ${cita}${fecha}]${avisoDerogacion}${avisoPlazo}\n${limpiarTexto(c.texto)}`;
    })
    .join("\n\n");
...
Cuando nombres un anexo, formulario o declaración jurada, respetá exactamente a qué producto, país y trámite corresponde según la fuente (por ejemplo, si el Anexo V es la declaración jurada de zarandeo de cebada, no lo atribuyas a la soja). No asumas que existe un anexo para un producto por analogía con otro: si la fuente no lo trae para lo que se consulta, decilo.
