...
- Pruebas: el 12/09/2026 se corrió una batería de 14 preguntas (`CUESTIONARIO.docx`) y se revisaron las respuestas a mano. Es una prueba interna, **no** feedback de exportadores o despachantes: la validación con usuarios reales sigue pendiente. Las 14 preguntas sirven como set de regresión (ver Pendientes).
...
- El modelo a veces completa con conocimiento propio cuando la base no tiene datos (por ejemplo, requisitos de Brasil o de España), y a veces presenta como vigentes instituciones o normas históricas (Ley 21.453, Res. SAGyP 1075/1994).
- Los sellos de normas que se muestran son los fragmentos recuperados, no necesariamente los citados en el texto de la respuesta.
- Respuestas orientativas: verificar siempre con SENASA y ARCA antes de operar.

## Pendientes

- [ ] Prompt del chat: reemplazar "Zarpe" por "Ceres AI"; usar ARCA (no AFIP); agregar glosario (ONPF = Organización Nacional de Protección Fitosanitaria, DJVE, SIM, NCM, FOB, GACC, "poroto" = poroto de soja); prohibir explícitamente completar con conocimiento externo; regla de alcance (Ganancias u otros temas fuera de la base); bajar `temperature` (0.3 dio detalles distintos entre dos corridas de la misma pregunta).
- [ ] Mostrar como sellos solo las normas citadas en el texto (o los 3-4 mejores), no todo lo recuperado.
- [ ] Búsqueda con filtro por país y cultivo detectados en la pregunta (más una búsqueda sin filtro); "trigo a Brasil" no recuperó la noticia ePhyto de Brasil, que sí está indexada.
- [ ] Encabezado con el título del documento en cada chunk antes de embeber (el registro de retenciones no se recuperó para "¿Qué dice el decreto 423/2026?").
- [ ] Revisar el registro de retenciones: explicitar el cronograma de maíz/sorgo (por tramos o mensual) para que el modelo no invente pasos intermedios.
- [ ] Verificar la fuente de la UE: la Directiva 2000/29/CE fue reemplazada por el Reglamento (UE) 2016/2031.
- [ ] Set de regresión: convertir las 14 preguntas de `CUESTIONARIO.docx` en un archivo con la fuente esperada y correrlo tras cada cambio.
- [ ] Validación con 2-3 exportadores o despachantes (antes de la preselección, 1-28/10).
