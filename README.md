...
**Zarpe** es el nombre de la app de chat (el "zarpe" es el documento que
autoriza a un buque a salir del puerto -- guiño directo al dominio: requisitos
para poder embarcar la exportación).
...
En vez del look genérico de "chatbot de IA" (fondo crema + acento terracota),
la estética de Zarpe se inspira en documentación de comercio exterior: papel
kraft, tipografía de ledger (Fraunces + IBM Plex Sans + IBM Plex Mono), y un
elemento de firma propio: cada norma citada se muestra como un **sello**
estampado (borde punteado, tinta roja, levemente rotado), como un sello de
aduana real -- en vez de una etiqueta plana como en Norma-AR.
...
- [ ] **Pendiente**: probar el flujo completo end-to-end con `vercel dev` (no se pudo ejecutar en este entorno de generación -- no hay forma de correr un navegador ni servir la función serverless acá). Antes de dar este paso por cerrado, correr `vercel dev`, hacer una pregunta real, y confirmar que: el streaming funciona, los sellos muestran la norma correcta, y el diseño se ve bien en mobile
- [ ] **Pendiente**: deploy real a Vercel con las variables de entorno cargadas
