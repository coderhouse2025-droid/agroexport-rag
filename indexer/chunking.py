...
# Filtro de alcance para el Repositorio SENASA (agregado 21/09/2026, ver diagnóstico
# en la sesión de esa fecha). senasa_repositorio.py busca por 8 keywords: 5 específicas
# de cultivo (soja, maiz, trigo, girasol, oleaginosas) y 3 genéricas (fitosanitario,
# granos, cereales) que devuelven CUALQUIER resolución que las mencione de pasada --
# incluye, por ejemplo, la aprobación de un saborizante para alimento animal a base de
# "proteína vegetal hidrolizada de soja" (Res. ex-SENASA 1039/1992), que no tiene nada
# que ver con requisitos de exportación de granos.
#
# Se conserva un documento si:
#   (a) entró por una keyword específica de cultivo (item["keyword_match"]), o
#   (b) su texto final (post-OCR) menciona alguno de los cultivos del alcance del
#       proyecto (CULTIVOS_CONOCIDOS, más abajo), aunque haya entrado por una keyword
#       genérica.
# Se descarta si entró SOLO por una keyword genérica y además no menciona ningún cultivo.
# Sobre 234 documentos del Repositorio (corrida del 21/09/2026): quedan 94, se
# descartan 140. cultivos_mencionados (calculado por downloader.py, ANTES del OCR) no
# se usa para esta decisión porque para los 75 documentos que necesitaron OCR quedó
# vacío por construcción (downloader.py corrió sobre texto que todavía no existía);
# en su lugar se recalcula sobre el texto final con _detectar_cultivos (ver más abajo).
KEYWORDS_CULTIVO_ESPECIFICO = {"soja", "maiz", "trigo", "girasol", "oleaginosas"}
...
def build_chunks_repositorio(items: list[dict], textos_dir: Path) -> tuple[list[dict], int, int]:
    chunks = []
    pendientes = 0
    descartados_fuera_de_alcance = 0
    for item in items:
        ...
        cultivos_en_texto = _detectar_cultivos(f"{item.get('titulo', '')} {texto}")
        entro_por_cultivo_especifico = item.get("keyword_match") in KEYWORDS_CULTIVO_ESPECIFICO
        if not entro_por_cultivo_especifico and not cultivos_en_texto:
            # Entró por una keyword genérica (fitosanitario/granos/cereales) y no
            # menciona ningún cultivo del alcance ni en el título ni en el texto
            # completo (post-OCR): fuera de alcance para este proyecto.
            descartados_fuera_de_alcance += 1
            continue
            ...
            chunks.append(
                {
                    ...
                    **meta_titulo,
                    ...
                    "fecha_iso": None,
                    "pais_destino": None,
                    "cultivos": cultivos_en_texto,
                    "url": item.get("item_url"),
                    "pdf_url": item.get("pdf_url"),
                    "extraido_via_ocr": bool(item.get("texto_via_ocr")),
                    "chunk_index": idx,
                    "total_chunks": len(partes),
                    "texto": parte,
                }
            )
    return chunks, pendientes, descartados_fuera_de_alcance
...
def main():
    ...
    chunks_repositorio, pendientes, descartados_alcance = [], 0, 0
    if repositorio_path.exists():
        items_repo = json.loads(repositorio_path.read_text(encoding="utf-8"))
        chunks_repositorio, pendientes, descartados_alcance = build_chunks_repositorio(items_repo, textos_dir)
    else:
    ...
    print(f"\nChunks del Repositorio: {len(chunks_repositorio)} (de documentos con texto disponible y dentro del alcance)")
    print(f"Documentos del Repositorio pendientes de texto (correr extract_text_local.py): {pendientes}")
    print(f"Documentos del Repositorio descartados por fuera de alcance (keyword genérica sin mención de cultivo): {descartados_alcance}")
