...
def a_registro_pinecone(chunk: dict) -> dict:
    ...
    for campo in (
        ...
        "cultivos", "url", "chunk_index", "total_chunks", "extraido_via_ocr",
        "vigente", "derogada_por_id", "derogada_por_titulo", "evidencia_derogacion",
    ):
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Borra TODOS los vectores del namespace antes de subir (gratis, no consume cuota de "
            "embeddings). Usar cuando chunks.json tiene MENOS documentos que la corrida anterior "
            "(ej. se agregó un filtro nuevo en chunking.py): sin esto, los chunks que salieron de "
            "chunks.json quedan como vectores huérfanos en Pinecone, todavía recuperables por la app."
        ),
    )
    ...
    if args.limpiar_antes:
        try:
            antes = index.describe_index_stats().namespaces.get(args.namespace, {}).get("vector_count", 0)
        except Exception:
            antes = "?"
        print(f"--limpiar-antes: borrando namespace '{args.namespace}' ({antes} vectores actuales) antes de subir...")
        try:
            index.delete(delete_all=True, namespace=args.namespace)
        except Exception as e:
            # con el namespace recién creado (índice nuevo) o ya vacío, delete_all puede
            # devolver 404 -- no es un error real, seguimos
    ...
    total = len(registros)
    subidos = 0
    fallidos_definitivos = []

    def subir_lote(lote: list[dict]):
        """Sube un lote con reintento por rate limit. Si falla por otro
        motivo (ej. un registro con contenido que rompe el parseo, como
        pasó el 15/09/2026 con un chunk de OCR muy sucio), Y el lote tiene
        más de 1 registro, lo divide a la mitad y reintenta cada mitad por
        separado -- así se aísla el/los registro(s) puntual(es) problemático(s)
        sin perder el resto del lote entero. Con lotes de 1, si sigue
        fallando, se da por perdido ESE registro únicamente y se sigue."""
        nonlocal subidos
        intentos = 0
        while True:
            try:
                ...
                if "RESOURCE_EXHAUSTED" in str(e) or "429" in str(e):
                    ...
                    time.sleep(espera)
                    continue
                # no es rate limit -- probablemente un registro con contenido problemático
                if len(lote) == 1:
                    print(f"  [SALTEADO, no se pudo subir] _id={lote[0]['_id']!r}: {e}")
                    fallidos_definitivos.append(lote[0]["_id"])
                    return
                mitad = len(lote) // 2
                print(f"  lote de {len(lote)} falló (no es rate limit), dividiendo en 2 para aislar el registro problemático...")
                subir_lote(lote[:mitad])
                subir_lote(lote[mitad:])
                return
    ...
    if fallidos_definitivos:
        print(f"\n[aviso] {len(fallidos_definitivos)} registro(s) no se pudieron subir bajo ningún tamaño de lote:")
        for _id in fallidos_definitivos:
            print(f"  - {_id}")
